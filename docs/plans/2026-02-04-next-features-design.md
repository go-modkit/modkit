# Next Features Plan (Option 1 + Companion Packages)

## Purpose
This document defines the post-MVP direction for modkit with a clear architectural north star. The core will remain Go-idiomatic and minimal (Option 1). Ergonomics and scaffolding will live in separate, optional packages (`modkitx` and `modkit-cli`). Implementation scheduling is intentionally deferred until the full repo review is complete.

## Architectural North Star: Option 1 (Go-Idiomatic Minimal Core)
...
The core must avoid runtime magic, decorators, and hidden conventions. It should be small, predictable, and debuggable. API changes should be conservative and motivated by clarity or correctness, not convenience. The goal is long-term stability and clear reasoning: a maintainer should be able to read the core and confidently predict behavior without implicit rules.
...
`modkitx` is an optional helper package that reduces boilerplate without changing semantics. It should only wrap or generate the same metadata that core consumes. It must not introduce reflection, auto-wiring, or hidden registration. The goal is convenience, not a new abstraction layer.
...
## Sequencing Notes
Implementation scheduling will be defined after the full review concludes. This document only fixes direction and package boundaries, not timeline or task breakdown.
