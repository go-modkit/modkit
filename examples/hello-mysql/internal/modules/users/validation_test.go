...
func TestController_CreateUser_Validation_Whitespace(t *testing.T) {
	svc := stubService{
		createFn: func(ctx context.Context, input CreateUserInput) (User, error) { return User{}, nil },
		listFn:   func(ctx context.Context) ([]User, error) { return nil, nil },
		updateFn: func(ctx context.Context, id int64, input UpdateUserInput) (User, error) { return User{}, nil },
	...
	body := []byte(`{"name":"   ","email":"ada@example.com"}`)
	req := httptest.NewRequest(http.MethodPost, "/users", bytes.NewReader(body))
	rec := httptest.NewRecorder()
	router.ServeHTTP(rec, req)
	...
	if len(problem.InvalidParams) != 1 || problem.InvalidParams[0].Name != "name" {
		t.Fatalf("expected invalidParams to include name, got %+v", problem.InvalidParams)
	}
}
...
func TestController_ListUsers_Validation(t *testing.T) {
	svc := stubService{
		createFn: func(ctx context.Context, input CreateUserInput) (User, error) { return User{}, nil },
		listFn:   func(ctx context.Context) ([]User, error) { return nil, nil },
		updateFn: func(ctx context.Context, id int64, input UpdateUserInput) (User, error) { return User{}, nil },
	...
	req := httptest.NewRequest(http.MethodGet, "/users?page=-1&limit=0", nil)
	rec := httptest.NewRecorder()
	router.ServeHTTP(rec, req)
...
func TestController_ListUsers_Validation_NonInteger(t *testing.T) {
	svc := stubService{
		createFn: func(ctx context.Context, input CreateUserInput) (User, error) { return User{}, nil },
		listFn:   func(ctx context.Context) ([]User, error) { return nil, nil },
		updateFn: func(ctx context.Context, id int64, input UpdateUserInput) (User, error) { return User{}, nil },
	...
	if len(problem.InvalidParams) != 1 || problem.InvalidParams[0].Name != "page" {
		t.Fatalf("expected invalidParams to include page, got %+v", problem.InvalidParams)
	}
}
